...
const fs = require('fs');
const log = require('better-logs')('mcd on margin');

const mcd = fs.readFileSync('./mcd/mcd.csv', 'utf8');
// const mcdSplit = fs.readFileSync('./mcd/mcd split.csv', 'utf8');
const mcdDividend = fs.readFileSync('./mcd/mcd-div.csv', 'utf8');
const ONE_DAY = 1000 * 60 * 60 * 24;

csvParse(mcd, { auto_parse: true, auto_parse_date: true, columns: true }, (mcdErr, result) => {
  // csvParse(mcdSplit, { auto_parse: true, auto_parse_date: true, columns: true }, (mcdSplitErr, result2) => {
    csvParse(mcdDividend, { auto_parse: true, auto_parse_date: true, columns: true }, (mcdDividendErr, result3) => {
      if (mcdErr || mcdDividendErr) {
        log.error('failed to parse.', mcdErr); //, mcdSplitErr, mcdDividendErr);
        process.exit(1);
      }
      ...
      for (let index = 0; index < result.length; index++) {
        ...
        if (div) {
          ...
          const divReturn = numStocks * div;
          leftover += divReturn;
          divEarnings += divReturn;
          elv += divReturn;
        }
